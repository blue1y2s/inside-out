...
export const PersonaDiffPanel: React.FC<PersonaDiffPanelProps> = ({ currentDimensions, targetPersona }) => {
  ...
  if (!currentDimensions) return <div className="text-neutral-charcoal/50 italic p-8">No seeds planted yet.</div>;
  if (!targetPersona) return <div className="text-neutral-charcoal/50 italic p-8">Select an ideal form to see growth gaps.</div>;
          ...
          return (
            <div key={key} className="relative group">
              <div className="flex justify-between items-end mb-2">
                <span className="text-sm font-bold uppercase tracking-widest text-neutral-charcoal/40">{key}</span>
                <span className={clsx("font-serif text-2xl", isAligned ? "text-neutral-charcoal/40" : (diff > 0 ? "text-emotion-joy" : "text-neutral-charcoal/60"))}>
              ...
              <div className="h-4 w-full bg-white rounded-full relative overflow-hidden border border-white shadow-inner">
                ...
                <div
                  ...
                  style={{
                    ...
                    backgroundColor: isAligned ? '#D6D3D1' : (diff > 0 ? '#FFD700' : '#FF6B35'), // Neutral, Joy, Anxiety
                  }}
                />
