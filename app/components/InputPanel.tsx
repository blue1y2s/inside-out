  ...
  return (
    <div className={`absolute top-0 left-0 h-full w-full md:w-[500px] glass-panel border-r border-white/50 p-10 flex flex-col z-20 shadow-2xl text-gallery-charcoal bg-white/40 transition-all duration-500 ease-in-out transform ${isExpanded ? 'translate-x-0 opacity-100 pointer-events-auto' : '-translate-x-10 opacity-0 pointer-events-none'}`}>
      ...
      <div className="mb-8">
        ...
        <p className="text-sm text-gallery-charcoal/60 font-sans tracking-wide uppercase">{t('input.subtitle')}</p>
      </div>

      {/* Preset Cards */}
      <div className="grid grid-cols-1 gap-3 mb-6">
        <p className="text-xs text-gallery-charcoal/50 font-bold uppercase tracking-widest mb-1">{t('input.preset.title')}</p>
        {PRESETS.map((p, i) => (
          <button
            key={i}
      ...
      <p className="text-gallery-charcoal/70 text-sm mb-2">{t('input.custom.title')}</p>
